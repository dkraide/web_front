...
export default function IngredienteLancamentoEstoqueForm({ user, isOpen, id, setClose, color }: props) {
    ...
    useEffect(() => {
        ...
        setValue('custoUnitario', ingrediente.valorCusto.toFixed(2));
        if (ingrediente.multiplicadorFornecedor <= 0) {
            ingrediente.multiplicadorFornecedor = 1;
        }
        setValue('multiplicador', ingrediente.multiplicadorFornecedor.toString())
    }, [ingrediente]);
    ...
    async function removeItem(produtoId) {
        ...
        var index = _.findIndex(produtos, p => p.materiaPrimaId == produtoId);
        if (index < 0) {
    ...
    function addItem() {
        if (!ingrediente || !ingrediente.id || ingrediente.id <= 0) {
        ...
        var ind = _.findIndex(produtos, p => p.materiaPrimaId == ingrediente.id);
        if (ind >= 0) {
    ...
    return (
        <BaseModal height={'90%'} width={'90%'} color={color} title={'Lancamento de Estoque de Ingrediente'} isOpen={isOpen} setClose={setClose}>
            {loading ? (
                <Loading />
            ) : (
                <div className={styles.container}>
                    ...
                    {lancamento.id > 0 ? (
                        ...
                        <div className={styles.box}>
                            ...
                            <SelectMateriaPrima ignore={ignore} width={'50%'} selected={ingrediente?.id || 0} setSelected={setIngrediente} />
