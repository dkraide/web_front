...
import SelectStatusVenda from "@/components/Selects/SelectStatusVenda"
...
export default function Venda() {
    ...
    const [search, setSearch] = useState<searchProps>({
        vendedorId: 0,
        dateIn: format(startOfMonth(new Date()), 'yyyy-MM-dd'),
        dateFim: format(endOfMonth(new Date()), 'yyyy-MM-dd'),
        movimentoCaixa: '',
        status: 0,
        statusSat: 0
    })
    ...
    const filtered = vendas.filter((v) => {
        const caixaId = fGetNumber(search.movimentoCaixa);
        if (caixaId > 0 && v.idMovimentoCaixa !== caixaId) {
            return false;
        }
        if (search.status > 0 && v.statusVenda !== (search.status == 1)) {
            return false;
        }
        return true;
    })
    ...
    const loadData = async () => {
        ...
        var url = `/Venda/List?empresaId=${user?.empresaSelecionada || u.empresaSelecionada}&dataIn=${search.dateIn}&dataFim=${search.dateFim}&vendedorId=${search.vendedorId}&movimentoCaixa=${search.movimentoCaixa}&status=${search.status}`;
        await api.get(url)
    ...
    const getTotalValue = (type: 'TOTAL' | 'CANCELADO' | 'FATURADO') => {
        let total = 0;
        switch (type) {
            case 'TOTAL':
                filtered.forEach((v) => {
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.boxSearch}>
                ...
                <SelectStatusVenda width={'300px'} selected={search.status} setSelected={(s) => {
                    setSearch({ ...search, status: s })
                }} />
            ...
            <div className={styles.boxTotais}>
                <BoxInfo value={GetCurrencyBRL(getTotalValue('TOTAL'))} title={'Total'} />
                <BoxInfo value={GetCurrencyBRL(getTotalValue('CANCELADO'))} title={'Cancelados'} />
                <BoxInfo value={GetCurrencyBRL(getTotalValue('FATURADO'))} title={'Faturados'} />
            </div>
            <hr />
